...
def sample_by_distance(df, sample_distance, output_file, report_file,
    col_lat, col_lon, col_group, Num_line):
    ...
    # Copiar
    lat_text = df[col_lat].copy()
    lon_text = df[col_lon].copy()
    ...
    # 2) Muestreo
    for (proj, lin), group in df.groupby([col_group, Num_line], sort=False):

        orig_idx = group.index.to_numpy()
        lons  = group[col_lon].values
        lats  = group[col_lat].values
    ...
    sampled_df[col_lat]  = lat_text.loc[sampled_idx].values
    sampled_df[col_lon] = lon_text.loc[sampled_idx].values
